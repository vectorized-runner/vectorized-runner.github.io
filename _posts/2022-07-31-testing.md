...
Now, we have one last important thing to remember: **You always need to pass the list with ref keyword to the methods:** 
...
void IncorrectListUsage()
{
	...
	// You also failed to Dispose the memory here, since on our local struct it's still null!
    list.Dispose();
}
...
void AddToListIncorrectly(ref StackOnlyList<int> list)
{
	list.Add(0);    
}
